				...
				((and argc param1 (& signal notUpd))
					...
					(if
						...
						(cond 
							(
								(and
									(== (gGameScript nextItem?) self)
									(!=
										(gGameScript nextAction?)
										((gIconBar helpIconItem?) message?)
									)
								)
								(gGameScript cue:)
							)
							((not temp1) (return 0))
							(else (gGameScript report:) (return 0))
						)
					)
					temp1
				)
...
(class IconBar of Set
	...
	(method (noClickHelp &tmp temp0 temp1 temp2 temp3 gLb2WinEraseOnly temp5)
		(= temp1 (= temp2 0))
		(= temp3 (GetPort))
		(= gLb2WinEraseOnly (gLb2Win eraseOnly?))
		(gLb2Win eraseOnly: 1)
		(while
		...
		(gLb2Win eraseOnly: gLb2WinEraseOnly)
		(gGame setCursor: 999 1)
		(if gDialog (gDialog dispose:))
		(SetPort temp3)
