...
(instance rm100 of LBRoom
	...
	(method (doit &tmp temp0)
		...
		(if script
			...
			(= temp0 (gOldFeatures firstTrue: #onMe gX (- gY 10)))
	...
	(method (handleEvent pEvent &tmp temp0 temp1 temp2)
		...
		(cond 
			(
				(or
					(and (== temp1 1) (not (pEvent modifiers?)))
			...
			((== temp0 1)
				(localproc_039e theFPlay 0)
				(if
				(>= (= temp2 (- (gOldFeatures indexOf: theFPlay) 1)) 0)
					(= theFPlay (gOldFeatures at: temp2))
				else
					(= theFPlay
						(gOldFeatures at: (- (gOldFeatures size?) 1))
					)
				)
			...
			((== temp0 5)
				...
				(if
					(<
						(= temp2 (+ (gOldFeatures indexOf: theFPlay) 1))
						(gOldFeatures size?)
					)
					(= theFPlay (gOldFeatures at: temp2))
				else
					(= theFPlay (gOldFeatures at: 0))
				)
				(localproc_039e theFPlay 1)
			)
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(gWrapSound number: 20 flags: 1 play:)
				(localproc_039e fIntro 0)
				(= ticks 40)
			)
			(2
			...
			(5
				(gWrapSound number: 23 flags: 1 play:)
		...
		(switch (= state newState)
			(0
				(if (== theFPlay fPlay)
					...
					(gWrapSound number: 100 loop: 1 flags: 1 play: self)
			...
			(18 (= local4 1))
			(19
				(global2 newRoom: (if (== theFPlay fPlay) 26 else 110))
			)
		)
	)
)
