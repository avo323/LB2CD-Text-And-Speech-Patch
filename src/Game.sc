...
(procedure (localproc_0e34 param1 &tmp temp0 [temp1 40] [temp41 40] temp81 [temp82 40] [temp122 10] [temp132 5])
...
(class Game of Obj
	...
	(method (init)
		...
		((= gLb2MDH mouseDownHandler) add:)
		((= gLb2KDH keyDownHandler) add:)
		((= gLb2DH directionHandler) add:)
		((= gLb2WH walkHandler) add:)
		(= gNewEventHandler 0)
		(= global29 (GetSaveDir))
		(Inv init:)
	...
	(method (doit &tmp newEvent thePanelObj thePanelSelector)
		...
		(if (== gNewRoomNumber gNumber) (gUser doit:))
		(gTheDoits doit:)
	...
	(method (replay &tmp temp0)
		...
		(cond 
			...
			(else (gGame setCursor: gWalkCursor))
		)
		(SL doit:)
		(DoSound sndRESTORE)
		(gSounds pause: 0)
	...
	(method (restore &tmp [temp0 20] temp20 temp21 [temp22 100] [temp122 5] [temp127 100])
		...
		(ScriptID 990)
		(= temp21 (self setCursor: gWalkCursor))
		(gSounds pause: 1)
		(if (localproc_0e34 1)
			(if gDialog (gDialog dispose:))
