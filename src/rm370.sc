...
(instance rm370 of LBRoom
	...
	(method (init &tmp [temp0 6])
		...
		(switch gGNumber
			...
			(355 0)
			(west
				(if (> (gEgo y?) 185) (gEgo y: 185))
			)
			(else  (gEgo posn: 160 160))
		)
		(super init:)
		(if (!= gGNumber east) (gGame handsOn:))
		(westExitFeature init:)
		(giftShoppeDoor init:)
		(if (> global123 2) (giftShoppeDoor locked: 1))
	...
	(method (doVerb theVerb)
		(if (== theVerb 4)
			(gLb2Messager
				say: noun theVerb (if (> global123 2) 1 else 2)
			)
		else
