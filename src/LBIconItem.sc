...
(class LBInvItem of InvI
	...
	(method (doVerb theVerb param2)
		...
		(cond 
			((and (> argc 1) param2) (gLb2Messager say: noun theVerb 0 0 0 15))
			((== theVerb 1) (gLb2Messager say: noun 1 0 0 0 15))
			((== theVerb 8) (gLb2Messager say: noun 8 0 0 0 15))
			((not (proc999_5 theVerb 3 4 2 6 12 13)) (gLb2Messager say: 0 47 0 0 0 15))
...
(instance invWin of InsetWindow
	...
	(method (open &tmp temp0 gInvFirst temp2)
		(= temp0 0)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(1
				...
				(gNarrator name: @local0 showTitle: 1)
				(cond 
					((== global150 4) (gLb2Messager say: 33 1 9 0 0 15))
					((== global150 3) (gLb2Messager say: 33 1 10 0 0 15))
					((== global150 2) (gLb2Messager say: 33 1 11 0 0 15))
					((== global150 1) (gLb2Messager say: 33 1 12 0 0 15))
					(else (gLb2Messager say: 33 1 8 0 0 15))
				)
			)
			(else  (super doVerb: theVerb))
		)
	...
	(method (doVerb theVerb)
		(switch theVerb
			(4
				(if (== cel 0)
					(if (global2 inset:)
						(gLb2Messager say: noun 4 4 0 0 15)
					else
						(gInv hide:)
						(global2 setScript: sCrankLantern)
					)
				)
			)
			(else  (super doVerb: theVerb))
		...
		(switch (= state newState)
			...
			(2
				...
				(gIconBar
					...
					enable: (gIconBar at: 5)
				)
				(lanternTimer setReal: lanternTimer 0 3)
				((gIconBar curIcon?) cursor: temp0)
				(gGame handsOn: setCursor: ((gIconBar at: 5) cursor?))
				(gIconBar select: (gIconBar at: 5))
				(self dispose:)
			)
		)
	)
...
(instance lanternTimer of Timer
	...
	(method (cue)
		(if (< gNumber 730)
			...
			(if (== (gIconBar curInvIcon?) lantern)
				((gIconBar useIconItem?) cursor: 84)
			)
			(if (== gCursorNumber 88) (gGame setCursor: 84))
		)
	)
)
